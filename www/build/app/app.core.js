...
(function(window, document, Context, namespace) {
  ...
  function queueUpdate(plt, elm) {
    ...
    if (!plt.isQueuedForUpdate.has(elm)) {
      ...
      // vdom diff and patch the host element for differences
            plt.queue.add(() => update(plt, elm), plt.isAppLoaded ? 1 /* Low */ : 3 /* High */);
    }
  }
  ...
  function initHostSnapshot(domApi, cmpMeta, elm) {
    true;
    ...
    cmpMeta.membersMeta && Object.keys(cmpMeta.membersMeta).forEach(memberName => {
      cmpMeta.membersMeta[memberName].attribName && (hostSnapshot.$attributes[memberName] = domApi.$getAttribute(elm, cmpMeta.membersMeta[memberName].attribName));
    });
    return hostSnapshot;
  }
  function connectedCallback(plt, cmpMeta, elm) {
    ...
    if (!plt.hasConnectedMap.has(elm)) {
      ...
      plt.requestBundle(cmpMeta, elm, initHostSnapshot(plt.domApi, cmpMeta, elm)), 3 /* High */);
    }
  }
